...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{./Sources/IMG_5155.jpg}
    \caption{ Fotografia przedstawiająca przebieg badania.}
    \label{fig:wykres_opoznienia}
\end{figure}
...
\subsubsection{Test T03: Wpływ procesu rejestracji na zasoby}
...
\subsubsection{Algorytm detekcji ruchu}
...
Wyniki testów wykazały, że system osiągnął średnią wydajność przetwarzania na poziomie \textbf{15,36 FPS}. Wartość ta przewyższa nominalną prędkość nadawania strumienia przez kamerę, wynoszącą 15 FPS. Oznacza to, że nadrzędny cel inżynierski został spełniony – zaimplementowany algorytm jest w stanie przetwarzać obraz na bieżąco, nie wprowadzając opóźnień w procesie przesyłu wideo, co jest kluczowe dla systemów monitoringu. Anomalia w postaci nadmiarowości wydajności stanowi prawdopodobnie o rezultat różnic w taktowaniu zegarów w momenci inicializacji i dezaktywacji aplikacji.

\paragraph*{Wysoki koszt obliczeniowy dla rozdzielczości Full HD}
Zauważono istotne obciążenie zasobów przy pracy z obrazem o wysokiej rozdzielczości. Wskazuje to jednoznacznie, że proces analizy każdej pojedynczej klatki w pełnej rozdzielczości 1920x1080 pikseli jest operacją wysoce wymagającą dla skryptu realizowanego w języku Python. W ramach późniejszego wdrożenia systemu na platformach o niższej mocy obliczeniowej (np. Raspberry Pi), konieczne będzie rozważenie optymalizacji algorytmu i przeniesienie krytycznych sekcji kodu do języków kompilowanych.
...
\subsubsection{Stabilność i płynność strumieniowania}
\label{subsubsec:wnioski_t02}

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{./Sources/latency_plot_updated.png}
    \caption{ Wykres opóźnienia end-to-end podczas testu T02.}
    \label{fig:wykres_opoznienia}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{./Sources/wykres_stabilnosci_fps_aktualizacja.png}
    \caption{ Wykres stabilności FPS podczas testu T02.}
    \label{fig:wykres_stabilnosci_fps}
\end{figure}
...
\subsubsection{Analiza wpływu procesu rejestracji na zasoby}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{./Sources/wykres_zuzycia_ram.png}
    \caption{ Wykres zużycia pamięci RAM podczas testu T03.}
    \label{fig:wykres_zuzycia_ram}
\end{figure}

\paragraph*{Dyskwalifikacja metody „Odroczonego Zapisu” w zastosowaniach ciągłych}
Zastosowana strategia buforowania całego materiału wideo w pamięci RAM (ang. \textit{In-Memory Buffering}), opisana w sekcji 2.5.11, okazała się nieprzydatna w warunkach produkcyjnych wymagających ciągłości działania. Przy odnotowanym tempie konsumpcji pamięci na poziomie \textbf{148,3 MB/s}, rozwiązanie to jest ograniczone funkcjonalnie wyłącznie do rejestracji bardzo krótkich sekwencji zdarzeń (ang. \textit{Short Clips}), takich jak kilkusekundowe klipy z detekcji ruchu. Metoda ta nie może być stosowana do monitoringu ciągłego (24/7).
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Płynność obrazu:} System osiągnął pełną wydajność przetwarzania (średnio 15,7 FPS), co potwierdza weryfikację pozytywną założenia o możliwości pracy w czasie rzeczywistym.
    ...
    Analiza wyników testu T01 na platformie referencyjnej (Intel Core i7-11370H) wykazała, że interpretowany język Python stanowi istotne wąskie gardło (ang. \textit{bottleneck}) dla przetwarzania obrazu o wysokiej rozdzielczości (Full HD $1920 \times 1080$ px). 
