...
\label{subsec:Analiza_technologii_narzedzi}
Wybór języka programowania Python oraz użyte biblioteki okazały się trafny z punktu widzenia dostępności zaawansowanych algorytmów przetwarzania obrazu. Python, jako język interpretowany, umożliwił szybkie iteracje kodu i łatwą integrację z istniejącymi bibliotekami. OpenCV dostarczyło bogaty zestaw narzędzi do analizy wideo, co znacząco skróciło czas potrzebny na rozwój funkcji wykrywania ruchu. Są to aspekty szczególnie istotne w kontekście prototypowania aplikacji badawczo-rozwojowych.

Jednakże, wybór Pythona wiązał się z pewnymi kompromisami wydajnościowymi, zwłaszcza w kontekście intensywnego przetwarzania strumieni wideo. W przyszłości, dla zastosowań wymagających większej wydajności, warto rozważyć \textbf{reimplementację krytycznych komponentów} w językach kompilowanych, takich jak C++ lub Rust.

Zarządzanie zależnościami za pomocą narzędzia \texttt{uv} okazało się skuteczne w utrzymaniu spójności środowiska programistycznego. Umożliwiło to łatwe odtwarzanie konfiguracji na różnych maszynach developerskich oraz stanowiskach testowych.

Konteneryzacja za pomocą Dockera okazała się kluczowa dla zapewnienia przenośności i izolacji środowiska uruchomieniowego. Umożliwiła łatwe wdrożenie aplikacji na różnych platformach bez konieczności ręcznej konfiguracji zależności. 
