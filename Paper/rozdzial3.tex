...
Do akwizycji obrazu wykorzystano bibliotekę \textbf{OpenCV} (klasa \texttt{cv2.VideoCapture}). Adres strumienia budowany jest dynamicznie w oparciu o poświadczenia, zgodnie ze schematem \texttt{rtsp://user:password@ip/stream1}. Równolegle inicjowana jest instancja obiektu \texttt{Tapo} z biblioteki pomocniczej \textbf{PyTapo}, która autoryzuje się w panelu administracyjnym kamery, umożliwiając wydawanie poleceń sterujących (\textit{PTZ}). 
...
\subsubsection*{Obsługa Audio i Separacja Logiczna}
\label{par:separacja_audio}

Warto zauważyć, że w przedstawionej klasie \texttt{TapoCamera} \textbf{celowo pominięto obsługę strumienia audio}. Zgodnie z przyjętą strategią niezawodności (\textit{Fault Tolerance}), obsługa dźwięku została wydzielona do \textbf{niezależnego obiektu} korzystającego z biblioteki \textbf{PyAV}. Taka separacja gwarantuje, że ewentualne opóźnienia w dekodowaniu dźwięku lub błędy formatu audio nie wpływają na płynność strumienia wideo i responsywność sterowania PTZ.
