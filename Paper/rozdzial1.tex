...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{L L L}
        \toprule
        \textbf{Cecha} & \textbf{Protokół TCP (\textit{Transmission Control Protocol})} & \textbf{Protokół UDP (\textit{User Datagram Protocol})} \\
        \midrule
        \textbf{Typ Połączenia} & Połączeniowy (wymaga nawiązania sesji) & Bezpołączeniowy (wysyła dane bez nawiązywania sesji) \\
        \midrule
        \textbf{Niezawodność} & Gwarantowana dostawa, kontrola błędów i kolejności & \enquote{Best-effort} (brak gwarancji dostawy i kolejności) \\
        \midrule
